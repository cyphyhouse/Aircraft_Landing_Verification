...
from data import get_dataloader_autoland
...
train_loader, val_loader = get_dataloader_autoland(args)
...
for epoch in range(args.epochs):
    ...
    # train for one epoch
    print('Epoch %d'%(epoch))
    _, _, _ = trainval(epoch, train_loader, writer=train_writer, training=True, alpha=args.alpha, _lambda=args._lambda)
    result_train, _, _ = trainval(epoch, train_loader, writer=None, training=False, alpha=args.alpha, _lambda=args._lambda)
    result_val, loss, prec = trainval(epoch, val_loader, writer=val_writer, training=False, alpha=args.alpha, _lambda=args._lambda)
    epoch += 1
    # if prec > best_prec:
    if loss < best_loss:
        best_loss = loss
