...
\n"
    ...
    x_est{i} = m.addVar(lb=-float('inf'), vtype=GRB.CONTINUOUS, name='x_est{i}')\n\
    y_est{i} = m.addVar(lb=-float('inf'), vtype=GRB.CONTINUOUS, name='y_est{i}')\n\
    z_est{i} = m.addVar(lb=-float('inf'), vtype=GRB.CONTINUOUS, name='z_est{i}')\n\
    yaw_est{i} = m.addVar(lb=-float('inf'), vtype=GRB.CONTINUOUS, name='yaw_est{i}')\n\
    pitch_est{i} = m.addVar(lb=-float('inf'), vtype=GRB.CONTINUOUS, name='pitch_est{i}')\n\
\n\
    ...
    sin_yaw_est{i} = m.addVar(lb=-1.1, ub=1.1, vtype=GRB.CONTINUOUS, name='sin_yaw_est{i}')\n\
    cos_yaw_est{i} = m.addVar(lb=-1.1, ub=1.1, vtype=GRB.CONTINUOUS, name='cos_yaw_est{i}')\n\
    sin_pitch_est{i} = m.addVar(lb=-1.1, ub=1.1, vtype=GRB.CONTINUOUS, name='sin_pitch_est{i}')\n\
    cos_pitch_est{i} = m.addVar(lb=-1.1, ub=1.1, vtype=GRB.CONTINUOUS, name='cos_pitch_est{i}')\n\
\n\
...
\n"
    ...
    # Dynamics for step {i}\n\
    m.addConstr(x_est{i}>=x{i}-23.339123)\n\
    m.addConstr(x_est{i}<=x{i}+23.339123)\n\
    m.addConstr(y_est{i}>=y{i}-4.873251)\n\
    m.addConstr(y_est{i}<=y{i}+4.873251)\n\
    m.addConstr(z_est{i}>=z{i}-2.01548)\n\
    m.addConstr(z_est{i}<=z{i}+2.01548)\n\
    m.addConstr(yaw_est{i}>=yaw{i}-0.001)\n\
    m.addConstr(yaw_est{i}<=yaw{i}+0.001)\n\
    m.addConstr(pitch_est{i}>=pitch{i}-0.001)\n\
    m.addConstr(pitch_est{i}<=pitch{i}+0.001)\n\
    ...
    m.addGenConstrSin(yaw_est{i}, sin_yaw_est{i})\n\
    m.addGenConstrCos(yaw_est{i}, cos_yaw_est{i})\n\
    m.addGenConstrSin(pitch_est{i}, sin_pitch_est{i})\n\
    m.addGenConstrCos(pitch_est{i}, cos_pitch_est{i})\n\
    m.addConstr(x_err{i}==cos_yaw_est{i}*({ref[0]}-x_est{i})+sin_yaw_est{i}*({ref[1]}-y_est{i}))\n\
    m.addConstr(y_err{i}==-sin_yaw_est{i}*({ref[0]}-x_est{i})+cos_yaw_est{i}*({ref[1]}-y_est{i}))\n\
    m.addConstr(z_err{i}=={ref[2]}-z_est{i})\n\
    m.addConstr(yaw_err{i}=={ref[3]}-yaw_est{i})\n\
...
\n\
    ...
    with open('gurobi_model_vcs.py','w+') as f:
        f.write(model_str)
