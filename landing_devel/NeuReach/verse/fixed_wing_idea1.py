...
if __name__ == "__main__":
    ...
    for step in range(10):
        ...
        state_space = scipy.spatial.Rectangle(state_high, state_low)
        partition = [state_space]
        for i in range(num_dim):
            prev = partition
            partition = []
            for region in prev:
                partition.extend(region.split(i, (state_high[i] + state_low[i])/2))
        ...
        for index in range(len(partition)): 
            state_low = partition[index].mins
            state_high = partition[index].maxes
            ...
            for i in range(num_sample):
                ...
                init_low = np.concatenate((state_low, state_low, ref))
                init_high = np.concatenate((state_high, state_high, ref))
