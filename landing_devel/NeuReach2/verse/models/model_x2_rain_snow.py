...
def compute_model_x(state_array, trace_array, E_array, pc=0.9, pr=0.95):
    X_process = state_array[:,0].reshape((-1,1))
    Y_process = trace_array[:,0]

    model_center = sm.QuantReg(Y_process, sm.add_constant(X_process))
    result = model_center.fit(q=0.5)
    cc = [result.params[1], result.params[0]]  

    X_process = state_array[:,0]
    X_radius = np.hstack((
        X_process.reshape((-1,1)),
        (X_process**2).reshape((-1,1))
    ))
    Y_radius = np.abs(Y_process - (cc[1] + X_process*cc[0]))
