...
def set_spotlight_properties(splotlight_angle: float) -> None:
    ...
    try:
        ...
        resp = set_light_properties_srv(
            ...
            cast_shadows=True,
            diffuse=ColorRGBA(80,80,80,255),
...
def get_complement(E):
    e1_range = np.round(np.arange(0.0, 1.0, 0.05),2)
    e2_range = np.round(np.arange(0.0, 0.02, 0.001),2) 
    E = np.round(E, 2)
    E_comp = []
    for e1 in e1_range:
        for e2 in e2_range:
            ...
            for e in E:
                if e1==e[0,0] and e2==e[0,1]:
                    ...
                    break
            if notin:
                E_comp.append(np.array([[e1, e2], [e1+0.05, e2+0.001]]))
...
def spawn_rectangle(name, rect):
    depth, width, height = rect[1,1:4] - rect[0,1:4]
    pos = (rect[1,1:4] + rect[0,1:4])/2

    # rospy.init_node("spawn_rectangle_node")

    # Generate the SDF description for the rectangle with specified dimensions
    model_xml = f"""
<?xml version='1.0'?>
<sdf version='1.6'>
  <model name='rectangle_model'>
    <link name='link'>
...
"""
    ...
    model_name = name  # Give a unique name if spawning multiple rectangles
    pose = Pose()  # Adjust position/orientation if necessary

    # Service client to spawn a model in Gazebo
...
if __name__ == "__main__":
    ...
    script_dir = os.path.dirname(os.path.realpath(__file__))
    with open(os.path.join(script_dir, 'exp1_res_safe_rain_snow_extend.pickle'), 'rb') as f:
        M, E, C_list = pickle.load(f)

    # for i, C_rect in enumerate(C_list):
    #     spawn_rectangle(f"rect{i}", C_rect)
    ...
    for i in range(30):
        ...
        with open('vcs_sim_exp1_safe_rain_snow_extend.pickle','wb+') as f:
            pickle.dump(traj_list, f)
        with open('vcs_estimate_exp1_safe_rain_snow_extend.pickle','wb+') as f:
            pickle.dump(estimate_traj_list, f)
        with open('vcs_init_exp1_safe_rain_snow_extend.pickle', 'wb+') as f:
