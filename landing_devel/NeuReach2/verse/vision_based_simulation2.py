...
class Perception:
    ...
    def set_percept(self, point: np.ndarray, e) -> np.ndarray:
        # Set aircraft to pos
        self.set_pos(point)
...
def run_vision_sim(scenario, init_point, init_ref, time_horizon, computation_step, time_step, vision_estimator: Perception, e):
    ...
    for t in time_points[1:]:
        estimate_point = vision_estimator.set_percept(point, e)
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        e = np.random.uniform(0.60, 0.70)
        set_light_properties(e)
        init_list.append((init_point, [e]))

        try:
            # Run simulation.
            traj, estimate_traj = run_vision_sim(fixed_wing_scenario, init_point, init_ref, time_horizon, 0.1, 0.01, vision, e=[np.random.uniform(0.4,0.5)])
