...
if __name__ == "__main__":
    ...
    fn = os.path.join(script_dir, './tmp.pickle')
    with open(fn, 'rb') as f:
        M, E, P, C_list = pickle.load(f)
    ...
    X0 = np.array([
        [-3030.0, -5, 100.0, 0-0.001, -np.deg2rad(3)-0.001, 10-0.01], 
        [-3000.0, 5, 140.0, 0+0.001, -np.deg2rad(3)+0.001, 10+0.01]
    ])
    X0_rect = X0[:,0:3]
    poly = pc.box2poly(X0_rect.T)
    plot_polytope_3d(poly.A, poly.b, ax, trans=0.3, edgecolor='k', color='#ffed6f')


    init_rect = P[:,0:3]
    poly = pc.box2poly(init_rect.T)
    plot_polytope_3d(poly.A, poly.b, ax, trans=0.1, edgecolor='k')
    ...
    with open(os.path.join(script_dir,'vcs_sim_exp1_unsafe.pickle'),'rb') as f:
        vcs_sim_trajectories = pickle.load(f)
    with open(os.path.join(script_dir,'vcs_estimate_exp1_unsafe.pickle'), 'rb') as f:
        vcs_sim_estimate = pickle.load(f)
    with open(os.path.join(script_dir,'vcs_init_exp1_unsafe.pickle'),'rb') as f:
