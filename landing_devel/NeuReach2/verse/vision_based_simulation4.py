...
if __name__ == "__main__":
    ...
    script_dir = os.path.dirname(os.path.realpath(__file__))
    with open(os.path.join(script_dir, 'tmp.pickle'), 'rb') as f:
        M, E, P, _ = pickle.load(f)
    state = P 
    ...
    for i in range(30):
        ...
        with open('vcs_sim_exp1_unsafe.pickle','wb+') as f:
            pickle.dump(traj_list, f)
        with open('vcs_estimate_exp1_unsafe.pickle','wb+') as f:
            pickle.dump(estimate_traj_list, f)
        with open('vcs_init_exp1_unsafe.pickle', 'wb+') as f:
