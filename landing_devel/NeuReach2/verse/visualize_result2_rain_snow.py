...
from fixed_wing13_rain_snow import pre_process_data, get_vision_estimation_batch
...
if __name__ == "__main__":
    ...
    with open(os.path.join(script_dir,'vcs_sim_exp1_safe_rain_snow.pickle'),'rb') as f:
        vcs_sim_trajectories = pickle.load(f)
    with open(os.path.join(script_dir,'vcs_estimate_exp1_safe_rain_snow.pickle'), 'rb') as f:
        vcs_sim_estimate = pickle.load(f)
    with open(os.path.join(script_dir,'vcs_init_exp1_safe_rain_snow.pickle'),'rb') as f:
    ...
    with open(os.path.join(script_dir,'vcs_sim_exp1_safecomp_rain_snow.pickle'),'rb') as f:
        vcs_sim_trajectories = pickle.load(f)
    with open(os.path.join(script_dir,'vcs_estimate_exp1_safecomp_rain_snow.pickle'), 'rb') as f:
        vcs_sim_estimate = pickle.load(f)
    with open(os.path.join(script_dir,'vcs_init_exp1_safecomp_rain_snow.pickle'),'rb') as f:
        vcs_sim_init = pickle.load(f)
    ...
    with open(os.path.join(script_dir, '../data_train_exp1_rain_snow.pickle'), 'rb') as f:
        data = pickle.load(f)
    ...
    #     e_plane[idx1, idx2] = 1
    # plt.imshow(e_plane)
    full_e = np.ones((20, 20))*(-1)
    min_acc = float('inf')
    min_acc_e1 = 0
    min_acc_e2 = 0
    for i in range(len(E)):
        E_part = E[i]
        idx1 = round((E_part[0,0]-0.0)/0.005)
        idx2 = round((E_part[0,1]-0.0)/0.005)
    ...
    rgba_image = np.zeros((20, 20, 4))  # 4 channels: R, G, B, A
    ...
    ax = plt.gca()
    ax.set_xticks(np.round(np.arange(0,22,2)-0.5,1))
    ax.set_yticks(np.round(np.arange(0,22,2)-0.5,1))
    ax.set_xticklabels(np.round(np.arange(0.0, 0.105, 0.01),2), fontsize=14)
    ax.set_yticklabels(np.round(np.arange(0.0, 0.105, 0.01),2), fontsize=14)
    plt.xlabel('Snow', fontsize=16)
